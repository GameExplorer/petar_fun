<template>
  <div class="mt-2 mb-2 font-bold text-center text-4xl">STATS</div>
  <div class="p-2 text-[1.475em]">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Race debut: <span class="font-semibold">1959, Netherlands</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Race starts: <span class="font-semibold">95</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Wins: <span class="font-semibold">0</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Poles: <span class="font-semibold">0</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Podiums: <span class="font-semibold">9</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Fastest laps: <span class="font-semibold">3</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Points: <span class="font-semibold">506</span></span
    >
  </div>
  <div class="p-2 text-[1.435em]">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Constructor's Championship: <span class="font-semibold">0</span></span
    >
  </div>
  <div class="p-2 text-3xl">
    <span class="px-2 py-1 hover:bg-white hover:text-[#006f62] hover:rounded-lg"
      >Driver's Championship: <span class="font-semibold">0</span></span
    >
  </div>
</template>
<script>
export default {
  name: "AstonMartinStats",
};
</script>
